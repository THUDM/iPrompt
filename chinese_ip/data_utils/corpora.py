...
class poem(PromptReader):
    PATH = "poems_save4"
    # PATH = "data/zhihu/data.json"
    assert_str = "make sure to set PATH for poem data_utils/corpora.py"
    qtitle_prefix = "题名:"
    qauthor_prefix = " 作者:"
    answer_prefix = " 正文: "
    ...
    def process_line(cls, data, tokenizer, tokenize):
        ...
        prompt = qtitle + cls.qauthor_prefix + qcontent + cls.qtitle_prefix + qtitle + cls.answer_prefix
        print(prompt,text)
        #text = data["ans-content"]
        prompt, text = cls.process_sample(prompt, tokenizer, tokenize), cls.process_sample(text, tokenizer,
                                                                                               tokenize)
